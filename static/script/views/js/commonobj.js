define(["jquery"],function(require){
	return commonObj = {
        ...
        getData: function (pagenumber) {
            ...
            $.ajax({
                type: "get",
                url: "/www/wxshop_mzxy/static/script/test.json",
                data: {
                    page:pagenumber,
                ...
                success: function (result) {
                	...
                	console.log(result.length/commonObj.pagesize);
                    $(".loaddiv").hide();
                    if (result.length > 0) {
                    	if(pagenumber<=result.length/commonObj.pagesize){
                    		 commonObj.ajaxstatus=true;
	                        commonObj.insertDiv(result);
	                        commonObj.loadCanvas();
                    	}else{
                    		$("#scrollAdd").append('<span style="color:red;text-align:center;line-height:30px;font-size:16px;display:block;">没有更多数据了....</span>')
                    	}
